getCN <- function(regionsInfo, readsInputs)
{
	...
	cnObject <- CNA(chrom = space(regionsInfo$inputs), maploc = start(regionsInfo$inputs), data.type = "logratio", genomdat = Mvalues, sampleid = paste(colnames(readsInputs)[2], "/", colnames(readsInputs)[1], "Fold Change"))
	...
	extends <- mapply(function(chr, end) {which(space(regionsInfo$inputs) == as.character(chr) & start(regionsInfo$inputs) == end)}, cnObject$out[, "chrom"], cnObject$out[, "loc.end"])
	cnObject$out[, "loc.end"] <- end(regionsInfo$inputs)[extends]   # Extend CNV region to the end of the interval, since all positions are starts.
