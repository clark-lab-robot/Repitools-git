checkProbes <- function(regionsTable, probesTable, outFile = "Probe Specificity")
{
	...
	pdf(paste(outFile, ".pdf", sep = ""), h = 8, w = 12)
	invisible(mapply(function(region, probesInRegion) {
		par(oma=c(4, 1, 1, 1))
		hitsMatch <- match(probesTable[probesInRegion, "name"], hits[, "name"])
		plot(x = probesTable[probesInRegion, "pos"], y = hits[hitsMatch, "number"], xlim = c(regionsTable[region, "start"], regionsTable[region, "end"]), yaxt="n", type = 'p', pch = 19, cex = 0.5, main = regionsTable[region, "name"], xlab = "Position", ylab = "Probe Hits", col = c("red", "green", "blue"))
