...
setMethod("cpgDensityPlot", "GenomeDataList", function(rs, cols=rainbow(length(rs)), xlim=c(0,20), main="CpG Density Plot", ...) {
...
setMethod("cpgDensityPlot", "GRangesList", function(rs, cols=rainbow(length(rs)), xlim=c(0,20), main="CpG Density Plot", ...) {
	if (length(cols)!=length(rs)) stop("rs and cols must have the same number of elements.")
	rs.cpg <- lapply(rs, cpgDensityCalc, ...)
	rs.den <- lapply(rs.cpg, density)
	ymax <- max(sapply(rs.den, function(u) max(u$y)))
