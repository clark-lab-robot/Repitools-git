setMethodS3("enrichmentPlot", "GenomeDataList", function(rs, organism, seqLen, cols=rainbow(length(rs)), xlim=c(0,20), main="Enrichment Plot", total.lib.size=TRUE, verbose=FALSE, ...) {
	if (length(cols)!=length(rs)) stop("rs and cols must have the same number of elements.")
	if (verbose) cat("Extending reads\n")
	rs.ext <- chipseq::extendReads(rs, seqLen=seqLen)
	if (verbose) cat("Creating coverage object\n")
	rs.cov <- BSgenome::gdapply(rs.ext, IRanges::coverage)
