...
annotationBlocksCounts <- function(rs, annotation, seqLen=NULL, verbose=TRUE) {
    ...
    anno.counts <- matrix(as.integer(NA), nrow=nrow(annotation), ncol=length(rs), dimnames=list(if(is.null(annotation$name)) rownames(annotation) else annotation$name, names(rs)))
